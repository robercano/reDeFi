 ...
 */
contract SiloVaultArkV2 is Ark {
        ...
        returns (address[] memory rewardTokens, uint256[] memory rewardAmounts)
    {
        address _gauge = IGaugeHookReceiver(
            ISiloConfig(silo.siloConfig()).getConfig(address(silo)).hookReceiver
        ).configuredGauges(address(silo));

        if (_gauge == address(0)) {
            revert InvalidGaugeAddress();
        }

        ISiloIncentivesController incentivesController = ISiloIncentivesController(
                _gauge
            );
