...
const OUT_DIR_SERVICE = path.join(__dirname, '../contracts-provider/service/src/generated')
const OUT_DIR_COMMON = path.join(__dirname, '../contracts-provider/common/src/generated')
...
function generateClass(
  ...
  const VERSION_DIR = path.join(OUT_DIR_SERVICE, `V${formattedVersion}`)
  const COMMON_VERSION_DIR = path.join(OUT_DIR_COMMON, `V${formattedVersion}`)
  ...
  const INTERFACES_DIR = path.join(COMMON_VERSION_DIR, 'interfaces')
  ...
  if (needsInterface) {
    ...
    interfaceContent += `import { TransactionInfo, AddressValue } from '@thesolidchain/sdk-common'\n\n`
  ...
  content += `import { IBlockchainClient } from '@thesolidchain/blockchain-client-common'\n`
  content += `import { IAddress, TransactionInfo, AddressValue } from '@thesolidchain/sdk-common'\n`
  content += `import { ${abiName} } from './abis/${abiName}'\n`
  content += `import { ${interfaceName} } from '@thesolidchain/contracts-provider-common'\n\n`
  ...
  content += `  public constructor(params: { blockchainClient: TClient; chainInfo: import('@thesolidchain/sdk-common').IChainInfo; address: TAddress }) {\n`
...
try {
  ...
  const versions = fs
    .readdirSync(OUT_DIR_SERVICE)
    .filter((d) => d.startsWith('V') && fs.statSync(path.join(OUT_DIR_SERVICE, d)).isDirectory())
    .sort((a, b) => {
  ...
  if (versions.length > 0) {
    ...
    factoryContent += `import { IBlockchainClient } from '@thesolidchain/blockchain-client-common'\n`
    factoryContent += `import { IAddress, IChainInfo } from '@thesolidchain/sdk-common'\n\n`
    ...
    // Generate unversioned exports linked to the latest version
    const latestV = versions[versions.length - 1]
    for (const contract of TARGET_CONTRACTS) {
      let unversionedContent = `// AUTOGENERATED FILE - DO NOT EDIT\n`
      unversionedContent += `export { ${contract}${latestV} as ${contract} } from './${latestV}/${contract}${latestV}'\n`
      unversionedContent += `export { ${contract}${latestV}Abi as ${contract}Abi } from './${latestV}/abis/${contract}${latestV}Abi'\n`
      fs.writeFileSync(path.join(OUT_DIR_SERVICE, `${contract}.ts`), unversionedContent)
      console.log(`Generated unversioned ${contract}.ts linked to ${latestV}`)

      let unversionedCommonContent = `// AUTOGENERATED FILE - DO NOT EDIT\n`
      unversionedCommonContent += `export type { I${contract}${latestV} as I${contract} } from './${latestV}/interfaces/I${contract}${latestV}'\n`
      if (!fs.existsSync(OUT_DIR_COMMON)) fs.mkdirSync(OUT_DIR_COMMON, { recursive: true })
      fs.writeFileSync(path.join(OUT_DIR_COMMON, `${contract}.ts`), unversionedCommonContent)
      console.log(`Generated unversioned common interface ${contract}.ts linked to ${latestV}`)
    }

    let commonIndexContent = `// AUTOGENERATED FILE - DO NOT EDIT\n`
    for (const v of versions) {
      for (const contract of TARGET_CONTRACTS) {
        commonIndexContent += `export type { I${contract}${v} } from './${v}/interfaces/I${contract}${v}'\n`
      }
    }
    for (const contract of TARGET_CONTRACTS) {
      commonIndexContent += `export type { I${contract} } from './${contract}'\n`
    }
    fs.writeFileSync(path.join(OUT_DIR_COMMON, 'index.ts'), commonIndexContent)
    console.log('Generated index.ts in common/src/generated')
  }
} catch (e) {
