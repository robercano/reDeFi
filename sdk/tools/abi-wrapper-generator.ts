...
try {
  ...
  if (versions.length > 0) {
    ...
    for (const contract of TARGET_CONTRACTS) {
      ...
      unversionedContent += `export type { I${contract}${latestV} as I${contract} } from './${latestV}/interfaces/I${contract}${latestV}'\n`
      unversionedContent += `export { ${contract}${latestV}Abi as ${contract}Abi } from './${latestV}/abis/${contract}${latestV}Abi'\n`
      fs.writeFileSync(path.join(OUT_DIR, `${contract}.ts`), unversionedContent)
