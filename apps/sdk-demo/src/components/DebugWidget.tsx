      ...
      {isOpen && (
        <div className="bg-black/95 border border-red-500 rounded-lg p-4 w-[90vw] max-w-md max-h-[60vh] overflow-y-auto text-xs font-mono text-red-400 shadow-2xl">
          <div className="flex justify-between items-center mb-2">
            ...
            <button onClick={() => setErrors([])} className="text-white bg-gray-800 px-2 rounded">Clear</button>
          </div>
          {errors.map((e, i) => (
            <div key={i} className="border-b border-red-900/50 py-1 break-words whitespace-pre-wrap">{e}</div>
          ))}
        </div>
      )}
    </div>
