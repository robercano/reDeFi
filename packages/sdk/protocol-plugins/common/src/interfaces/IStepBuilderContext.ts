...
export interface IStepBuilderContext {
   ...
   * @param params.transaction A full formed transaction
   */
  addTransaction(params: { transaction: TransactionInfo }): void

  /**
   * Adds an action call to the context
   * @param params.step The step to which the action belongs
   * @param params.action The action to be called
   * @param params.arguments The arguments to be passed to the action
   * @param params.connectedInputs The connected inputs to the action, this is the values that the action
   *                        will read from storage
   * @param params.connectedOutputs The connected outputs to the action, this is the values that the action
   *                         will write to storage
   * @param params.skip If true, the action will be skipped and empty calldata will be added to the TX
   */
  addActionCall<
