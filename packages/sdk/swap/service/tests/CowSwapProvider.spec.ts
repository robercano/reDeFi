...
import { IntentSwapProviderType, ChainIds, Address, Token, ChainInfo, TokenAmount } from '@thesolidchain/sdk-common'
...
describe('CowSwapProvider', () => {
  ...
  it('should throw error when checking supported chain ID if invalid', () => {
    const provider = new CowSwapProvider({ configProvider, allowanceManager, tokensManager })
    expect(() => (provider as any)._assertSupportedChainId(9999)).toThrow(/not supported by CowSwapProvider/)
  })
  ...
  it('should get sell order quote correctly', async () => {
    ...
    const mockSender = Address.createFromEthereum({ value: '0x3333333333333333333333333333333333333333' })
