...
import { IPercentage, ChainIds, SwapProviderType, Token, ChainInfo, Address, TokenAmount } from '@thesolidchain/sdk-common'
...
describe('OneInchSwapProvider', () => {
  ...
  it('should format url and fetch swap data correctly', async () => {
    ...
    const mockToken = Token.createFrom({ address: Address.createFromEthereum({ value: '0x1111111111111111111111111111111111111111' }), chainInfo, symbol: 'ABC', name: 'ABC', decimals: 18 })
    const mockFromAmount = TokenAmount.createFrom({ token: mockToken, amount: '1000' })
    const mockToToken = Token.createFrom({ address: Address.createFromEthereum({ value: '0x2222222222222222222222222222222222222222' }), chainInfo, symbol: 'DEF', name: 'DEF', decimals: 18 })
    const mockRecipient = Address.createFromEthereum({ value: '0x3333333333333333333333333333333333333333' })
    ...
    mockFetch.mockResolvedValue({
      ...
      json: async () => ({
        toTokenAmount: '900',
        tx: { data: '0xdata', to: '0x4444444444444444444444444444444444444444', value: '0', gasPrice: '10' }
      })
    } as any)
  ...
  it('should handle fetch errors in swap data', async () => {
    ...
    await expect(provider.getSwapDataExactInput({
      fromAmount: mockFromAmount,
      toToken: mockToToken,
      recipient: mockRecipient,
      slippage: mockSlippage
